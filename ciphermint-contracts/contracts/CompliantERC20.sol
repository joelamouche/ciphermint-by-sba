...
contract CompliantERC20 is ZamaEthereumConfig, Ownable2Step {
    ...
    mapping(address account => euint64 balance) internal balances;

    /// @notice Encrypted allowances
    mapping(address owner => mapping(address spender => euint64 allowance)) private allowances;
    // UBI-specific state (claim status and lastIncomeBlock) moved to CompliantUBI
    ...
    /**
     * @notice Internal mint helper to add encrypted amount to a balance
     * @param to Recipient address
